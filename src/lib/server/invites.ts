...
/** Create one invite owned by `userId` and return its code. */
export async function createInviteReturningCode(userId: string): Promise<string> {
	const code = generateInviteCode();
	await db.insert(invite).values({ id: code, createdByUserId: userId });
	return code;
}
